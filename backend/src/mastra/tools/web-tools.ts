...
/**
 * Shared fetch implementation used by both fetchPageTool (investigate agent)
 * and buildExtractTool (programmatic extraction). Exported so extract-tool.ts
 * can call it directly without going through a Mastra tool boundary.
 */
export async function executeFetchPage(targetUrl: string): Promise<{
  title?: string;
  text?: string;
  error?: string;
}> {
...
export const fetchPageTool = createTool({
  ...
  execute: async ({ url: targetUrl }) => executeFetchPage(targetUrl),
});
